...
//	int maxNNodes = 1000;
	...
	public Tree<CallgraphState> run(int nnodes){
        ...
        while (queue.size() > 0 && tree.nodeMap.size() <= nnodes){
            ...
            print("################# ", String.valueOf(cn.value.method));
            print(String.valueOf(calledMethodsWithUnit));
            print("########################\n");
...
//					print("****** add into queue:", String.valueOf(nn), " BECAUSE:", "targetClass.isApplicationClass():",
//							String.valueOf(targetClass.isApplicationClass()), "isSupportClass:", isSupportClass(targetClass), "targetClass.getName():", targetClass.getName(), String.valueOf(tree.nodeMap.size()));
					queue.add(nn);
				}
            }
        }
